ccs_54061<-read.csv("Y_54061_single.csv",header = F)
names(ccs_54061)<-c("pos","wt","mut")

ccs2_54061<-ccs_54061 %>%
  group_by(pos,wt,mut) %>%
  dplyr::mutate(Freq_ccs=sum(pos)/pos)%>%unique()

ccs3_54061 <- ccs2_54061 %>%
  group_by(pos) %>%
  dplyr::summarise(Freq_ccs=sum(Freq_ccs))  


names(ccs2_54061)<-c("pos","wt","mut","Freq")
ggplot(ccs2_54061,aes(x=pos,y=Freq,color = mut))+ geom_point(size=1)+facet_grid( mut ~.)+scale_x_continuous(breaks=seq(0, 1000, 50),expand = c(0, 0))

ggsave("2-18ccs_singlepos_point.png")


ccs_54136<-read.csv("Y_54136_single.csv",header = F)
names(ccs_54136)<-c("pos","wt","mut")

ccs2_54136<-ccs_54136 %>%
  group_by(pos,wt,mut) %>%
  dplyr::mutate(Freq_ccs=sum(pos)/pos)%>%unique()

ccs3_54136 <- ccs2_54136 %>%
  group_by(pos) %>%
  dplyr::summarise(Freq_ccs=sum(Freq_ccs))  


names(ccs2_54136)<-c("pos","wt","mut","Freq")
ggplot(ccs2_54136,aes(x=pos,y=Freq,color = mut))+ geom_point(size=1)+facet_grid( mut ~.)+scale_x_continuous(breaks=seq(0, 1000, 50),expand = c(0, 0))
